...
export const getOneRequest = (object = {}) => {
    ...
    return dispatch => {
        if (!data.id) {
            ...
            return api.get({
                url: apiUrl.API_NHAN_SU + `/${data.id}`,
                controller
            }).then(res => {
                if (res && res.status) {
...
export const getDetailRequest = (object = {}) => {
    var { data, requestSuccess, requestError, controller } = main.checkValidObjectRequest(object);
    return dispatch => {
        dispatch(getOneRequest(object));
        if (!data.id) {
            message.error({ content: "Dữ liệu đầu vào không chính xác!" });
        }
        else {
            return api.get({
                url: apiUrl.API_NHAN_SU_DETAIL(data.id),
                controller
            }).then(res => {
                if (res && res.status) {
                    dispatch(handleGetDetail(res.result));
                    requestSuccess();
                }
                else {
                    requestError();
