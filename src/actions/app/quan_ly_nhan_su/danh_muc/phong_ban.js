...
export const getAllRequest = (object = {}) => {
    ...
    return dispatch => {
        return api.get({
            url: apiUrl.API_PHONG_BAN_NHAN_SU,
            data,
            controller
        }).then(res => {
            if (res && res.status) {
...
export const getOneRequest = (object = {}) => {
    ...
    return dispatch => {
        if (!data.id) {
            ...
            return api.get({
                url: apiUrl.API_PHONG_BAN_NHAN_SU + `/${data.id}`,
                controller
            }).then(res => {
                if (res && res.status) {
...
const handleCreate = (value) => {
    return {
        type: type.TYPE_PHONG_BAN_CREATE,
        value
    }
}
...
const handleUpdate = (value) => {
    return {
        type: type.TYPE_PHONG_BAN_UPDATE,
        value
    }
}
...
const handleDelete = (values) => {
    return {
        type: type.TYPE_PHONG_BAN_DELETE,
        values
    }
}