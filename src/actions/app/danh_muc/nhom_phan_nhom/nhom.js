...
export const getOneRequest = (object = {}) => {
    ...
    return dispatch => {
        if (!data.id) {
            ...
            return api.get({
                url: apiUrl.API_TCB_NHOM + `/${data.id}`,
                controller
            }).then(res => {
                if (res && res.status) {
