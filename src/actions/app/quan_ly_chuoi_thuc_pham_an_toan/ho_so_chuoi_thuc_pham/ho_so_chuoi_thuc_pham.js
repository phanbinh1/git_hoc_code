...
export const getOneRequest = (object = {}) => {
    ...
    return dispatch => {
        if (!data.id) {
            ...
            return api.get({
                url: apiUrl.API_HO_SO_CHUOI_THUC_PHAM + `/${data.id}`,
                controller
            }).then(async res => {
                if (res && res.status) {
                    let { result } = res;
...
const handleGetAll = (values) => {
    return {
        type: type.TYPE_HO_SO_CHUOI_THUC_PHAM_LIST,
        values
    }
}
...
export const handleGetOne = (value) => {
    return {
        type: type.TYPE_HO_SO_CHUOI_THUC_PHAM_EDIT,
        value
    }
}
...
const handleCreate = (value) => {
    return {
        type: type.TYPE_HO_SO_CHUOI_THUC_PHAM_CREATE,
        value
    }
}
...
const handleUpdate = (value) => {
    return {
        type: type.TYPE_HO_SO_CHUOI_THUC_PHAM_UPDATE,
        value
    }
}
...
const handleDelete = (values) => {
    return {
        type: type.TYPE_HO_SO_CHUOI_THUC_PHAM_DELETE,
        values
    }
}

export const getSoGXNRequest = (object = {}) => {
    var { requestSuccess, requestError, controller } = main.checkValidObjectRequest(object);
    return dispatch => {
        return api.get({
            url: apiUrl.API_HO_SO_CHUOI_THUC_PHAM + `/goiysoxacnhanchuoithucphamantoan`,
            controller
        }).then(res => {
            if (res && res.status) {
                requestSuccess(res);
