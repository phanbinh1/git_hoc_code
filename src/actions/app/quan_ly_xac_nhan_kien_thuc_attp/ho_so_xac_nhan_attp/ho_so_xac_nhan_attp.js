...
export const getAllRequest = (object = {}) => {
    ...
    return dispatch => {
        return api.get({
            url: apiUrl.API_XAC_NHAN_KIEN_THUC_ATTP_HSXN,
            data,
            controller
        }).then(res => {
            if (res && res.status) {
...
export const getOneRequest = (object = {}) => {
    ...
    return dispatch => {
        if (!data.id) {
            ...
            return api.get({
                url: apiUrl.API_XAC_NHAN_KIEN_THUC_ATTP_HSXN + `/${data.id}`,
                controller
            }).then(res => {
                if (res && res.status) {
...
export const getDSHoSoRequest = (object = {}) => {
    var { data, requestSuccess, requestError, controller } = main.checkValidObjectRequest(object);
    return () => {
        if (!data.idCoSo) {
            message.error({ content: "Dữ liệu đầu vào không chính xác!" });
            ...
            return api.get({
                url: apiUrl.API_DS_HO_SO_XAC_NHAN_KIEN_THUC_BY_ID_CO_SO(data.idCoSo),
                controller
            }).then(res => {
                if (res && res.status) {
                    requestSuccess(res);
...
export const capSoRequest = (object = {}) => {
    ...
    return dispatch => {
        return api.put({
            url: apiUrl.API_XAC_NHAN_KIEN_THUC_ATTP_HSXN_CAP_SO,
            data,
            controller
        }).then(res => {
            if (res && res.status) {
...
const handleGetAll = (values) => {
    return {
        type: type.TYPE_XNKT_ATTP_HO_SO_XAC_NHAN_LIST,
        values
    }
}
...
export const handleGetOne = (value) => {
    return {
        type: type.TYPE_XNKT_ATTP_HO_SO_XAC_NHAN_EDIT,
        value
    }
}
...
const handleCreate = (value) => {
    return {
        type: type.TYPE_XNKT_ATTP_HO_SO_XAC_NHAN_CREATE,
        value
    }
}
...
const handleUpdate = (value) => {
    return {
        type: type.TYPE_XNKT_ATTP_HO_SO_XAC_NHAN_UPDATE,
        value
    }
}
...
const handleDelete = (values) => {
    return {
        type: type.TYPE_XNKT_ATTP_HO_SO_XAC_NHAN_DELETE,
        values
    }
}