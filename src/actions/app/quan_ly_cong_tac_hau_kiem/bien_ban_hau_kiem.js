...
export const getListByIdHoSo = (object = {}) => {
    ...
    return () => {
        if (!data.idHoSo) {
            ...
            return api.get({
                url: apiUrl.API_HO_SO_TU_CONG_BO_DS(data.idHoSo),
                controller
            }).then(res => {
                if (res && res.status) {
                    requestSuccess(res);
...
export const getOneRequest = (object = {}) => {
    ...
    return dispatch => {
        if (!data.id) {
            ...
            return api.get({
                url: apiUrl.API_HO_SO_TU_CONG_BO_BIEN_BAN_HAU_KIEM + `/${data.id}`,
                controller
            }).then(res => {
                if (res && res.status) {
                    requestSuccess(res);
...
export const createRequest = (object = {}) => {
    ...
    return () => {
        return api.post({
            url: apiUrl.API_HO_SO_TU_CONG_BO_BIEN_BAN_HAU_KIEM,
            data,
            controller
        }).then(res => {
            if (res && res.status) {
