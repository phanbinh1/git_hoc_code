...
const ListFilter = ({
    ...
    const trangThaiDuyet = qs.trang_thai_phe_duyet && CONST_PHE_DUYET.thanhTraOptions.findIndex(item => item.num_key === qs.trang_thai_phe_duyet) >= 0 ? CONST_PHE_DUYET.thanhTraOptions.find(item => item.num_key === qs.trang_thai_phe_duyet).value : undefined;
    ...
    const getSelectedKeys = () => {
        let result = [];
        qs.trang_thai_phe_duyet && result.push(`trang_thai_phe_duyet${qs.trang_thai_phe_duyet}`);
        result.length === 0 && result.push("ALL");
        return result;
    }
    return <Fragment>
        {!qs.loaded && !qs.nam && <Redirect to={{ pathname: location.pathname, hash: location.hash, search: queryString.stringify({ ...qs, nam: moment().year(), loaded: true }) }} />}
        <CommonFilter
            menus={[
                { label: "Tất cả kế hoạch", key: "ALL", iconCls: "fa-reply-all" },
                { type: "divider" },
                { isTitle: true, iconCls: "fa-pencil-square-o", label: "Trạng thái phê duyệt", disabled: true },
                ...CONST_PHE_DUYET.thanhTraOptions.map(item => ({ key: `${item.name}${item.num_key}`, label: item.label, ...item })),
            ]}
            selectedKeys={getSelectedKeys()}
            onSelect={(key, item) => {
                if (key === "ALL") {
                    history.push({ search: queryString.stringify({ ...qs, ...{ trang_thai_phe_duyet: undefined } }) })
                }
                else {
        ...
        <Dropdown
            ...
            overlay={<Menu
                selectedKeys={[`${nam}`]}
                onClick={_data => {
                    history.push({ search: queryString.stringify({ ...qs, nam: _data.key }) })
                }}>
