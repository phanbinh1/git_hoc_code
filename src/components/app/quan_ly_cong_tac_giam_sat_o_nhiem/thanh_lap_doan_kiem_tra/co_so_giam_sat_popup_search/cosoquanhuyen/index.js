...
const ListQuanHuyen = ({
    ...
    useEffect(() => {
        setFirstLoad(false);
        _setRowSelected(coSoSelected && Array.isArray(coSoSelected) ? coSoSelected.filter(item => item.loaiCoSo === constants.CONST_LOAI_CO_SO.COSO_QUANHUYEN) : []);
    }, [])
                    ...
                    dataSource.map((item) => {
                        ...
                        if (index !== -1) {
                            if (rowSelected.findIndex(row => row.idCoSo === item.idCoSo) === -1) {
                                rowSelected.push({
                                    ...item,
                                    diaDiemKinhDoanh: item.diaChiCoSo,
                                });
                            }
                        }
                        else {
    ...
    const onSearch = () => {
        if (search.maQuanHuyen) {
            ...
            getDsCoSo({
                ...
                requestSuccess: (res) => {
                    setDataSource(res.result.map(item => ({
                        ...item,
                        idCoSo: item.id,
                        tinhThanh: constants.CONST_DEFAULT_TINHTHANH,
                        quanHuyen: dsQuanHuyen.find(qh => qh.ma === item.maQuanHuyen),
                        xaPhuong: dsXaPhuong.find(xp => xp.ma === item.maPhuong),
                    })));
                    setLoading(false);
                },
                requestError: () => {
    ...
    return <React.Fragment>
        <FormSearch
            ...
            setDsQuanHuyen={setDsQuanHuyen}
            dsXaPhuong={dsXaPhuong}
            setDsXaPhuong={setDsXaPhuong}
        />
        <Table
            bordered
            size="small"
            ...
            columns={[
                ...
                { dataIndex: "_diaChiCoSo", title: "Địa chỉ", className: "c-pointer" },
            ...
            dataSource={dataSource.map((item, index) => {
                const _diaChiCoSo = `${item.diaChiCoSo && typeof item.diaChiCoSo === "string" ? item.diaChiCoSo : ""}` +
