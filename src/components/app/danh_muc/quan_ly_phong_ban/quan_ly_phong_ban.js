...
import QuanLyPhongBanList from "./quan_ly_phong_ban_list";
import QuanLyPhongBanForm from "./quan_ly_phong_ban_form";
...
const QuanLyPhongBan = (props) => {
    ...
    const renderActionCreate = () => {
        let result = {};
        result.key = actID.ACT_ID_QUAN_LY_PHONG_BAN.ACT_CREATE;
    ...
    const renderActionDelete = () => {
        ...
        result.key = actID.ACT_ID_QUAN_LY_PHONG_BAN.ACT_DELETE;
    ...
    const renderActionSearch = () => {
        let result = {};
        result.key = actID.ACT_ID_QUAN_LY_PHONG_BAN.ACT_SEARCH;
        result.disabled = false;
        result.hidden = (isVisiableList && !isVisiableSearch) ? false : true;
    ...
    return (
        <React.Fragment>
            {isVisiableForm && <QuanLyPhongBanForm  {...props} />}
            {isVisiableList && <QuanLyPhongBanList {...props} />}
        </React.Fragment>
    );
}
