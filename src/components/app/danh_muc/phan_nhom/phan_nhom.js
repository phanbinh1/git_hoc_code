...
import PhanNhomList from "./phan_nhom_list";
import PhanNhomForm from "./phan_nhom_form";
...
const PhanNhom = (props) => {
    ...
    const renderActionCreate = () => {
        let result = {};
        result.key = actID.ACT_ID_TCB_PHAN_NHOM.ACT_CREATE;
    ...
    const renderActionDelete = () => {
        ...
        result.key = actID.ACT_ID_TCB_PHAN_NHOM.ACT_DELETE;
    ...
    const renderActionSearch = () => {
        let result = {};
        result.key = actID.ACT_ID_TCB_PHAN_NHOM.ACT_SEARCH;
        result.disabled = false;
        result.hidden = (isVisiableList && !isVisiableSearch) ? false : true;
    ...
    return (
        <React.Fragment>
            {isVisiableForm && <PhanNhomForm  {...props} />}
            {isVisiableList && <PhanNhomList {...props} />}
        </React.Fragment>
    );
}
