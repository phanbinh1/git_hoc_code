...
import NhomList from "./nhom_list";
import NhomForm from "./nhom_form";
...
const Nhom = (props) => {
    ...
    const renderActionCreate = () => {
        let result = {};
        result.key = actID.ACT_ID_TCB_NHOM.ACT_CREATE;
    ...
    const renderActionDelete = () => {
        ...
        result.key = actID.ACT_ID_TCB_NHOM.ACT_DELETE;
    ...
    const renderActionSearch = () => {
        let result = {};
        result.key = actID.ACT_ID_TCB_NHOM.ACT_SEARCH;
        result.disabled = false;
        result.hidden = (isVisiableList && !isVisiableSearch) ? false : true;
    ...
    return (
        <React.Fragment>
            {isVisiableForm && <NhomForm  {...props} />}
            {isVisiableList && <NhomList {...props} />}
        </React.Fragment>
    );
}
