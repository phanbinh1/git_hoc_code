...
import {
    ...
    VALIDATE_QTNVTT_KEHOACHTHANHTRA_TRANG_THAI_REQUIRED
} from "./../../../constants/validate"
import { FieldDate, FieldInput, FieldSelect } from "./../../../constants/controll";
    ...
    return <Fragment>
        <Modal
            ...
            footer={[
                ...
                ...(onBanHanh && allowBanHanh && !readOnly ? [<Button
        ...
        >
            <CommonForm
                    ...
                    [
                        {
                            ...
                            renderCustom: <BanHanh
                                ...
                                fields={[
                                    ...fields,
                                    ...(
                                        onBanHanh && allowBanHanh && !readOnly ? [
                                            {
                                                ...
                                                checkValid: true,
                                                validate: [VALIDATE_QTNVTT_KEHOACHTHANHTRA_TRANG_THAI_REQUIRED]
                                            }
                                        ] : []
                                    )
                                ]}
                                readOnly={!Boolean(onBanHanh)}
                            />
                        }
                    ]
                ]}
                ...
                onSubmit={(values) => {
                    onBanHanh && Modal.confirm({
                        title: banHanhConfirmLabel || `Bạn chắc chắn muốn ban hành`,
                        onOk: () => {
                            setLoading(true);
                            onBanHanh({
                                data: values,
                            ...
                            <Form {...layout}>
                                {
                                    fields.map((field, index) => {
                                        return <Form.Item label={<span className={field.checkValid ? "ant-form-item-required" : ""} > {field.label}</span>} key={index}>
                                            <Field {...field} label={undefined} readOnly={readOnly} />
                                        </Form.Item>
                                    })
                                }
                            </Form>
