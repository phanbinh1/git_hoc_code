...
import { ThemMoiCongViecVanBanDieuHanh_HoSoThamDinhBanHanhCongVan } from "./../../../../actions/app/van_ban_dieu_hanh";
...
const TrinhKy = ({
    ...
    useEffect(() => {
        if (visible && entityId) {
            ...
            setTenHoSo(`${CONST_VBDH_CONFIG.CVHSTDCGCN.tenHoSo} ${entityId}`);
        }
    }, [entityId, visible]);
    ...
    const [tenHoSo, setTenHoSo] = useState(`${CONST_VBDH_CONFIG.CVHSTDCGCN.tenHoSo} ${entityId}`);
    ...
    const findAccount = async (email) => {
        const res = await get({ url: API_ACCOUNT, data: { emailSearch: email }, errorNotifi: false });
        setAccounts(main.deduplicate(res.status ? res.result : [], "email"));
    }
    ...
    const onOk = async () => {
        ...
        const maHoSo = `${identityCode}.${moment().format("YYMMDD")}.${CONST_VBDH_CONFIG.CVHSTDCGCN.maHoSo}${entityId}`;
        ...
        const res = await ThemMoiCongViecVanBanDieuHanh_HoSoThamDinhBanHanhCongVan(data);
        if (res.result && res.result.error === 0) {
            message.success({ content: "Đã trình ký văn bản" });
            callback && callback({
                status: true,
                keHoachTienHanhCuocThanhTra: res.keHoachTienHanhCuocThanhTra
            })
            _onCancel();
        }
        else {
                ...
                <div className="ant-form-inline">
                    ...
                    <Form.Item labelCol={{ span: 8 }} label="Email người nhận" required>
                        <AutoComplete
                            ...
                            dataSource={accounts.map(item => <AutoComplete.Option key={item.id} value={item.email} text={item.email} {...item}>
                                {item.fullName}
                            </AutoComplete.Option>)}
