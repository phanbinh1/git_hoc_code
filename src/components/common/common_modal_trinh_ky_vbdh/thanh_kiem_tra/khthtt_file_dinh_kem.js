...
import { ThemMoiCongViecVanBanDieuHanh_KeHoachTienHanhCuocThanhKiemTra } from "./../../../../actions/app/van_ban_dieu_hanh";
...
const TrinhKy = ({
    ...
    useEffect(() => {
        if (visible && entityId) {
            ...
            setTenHoSo(`${CONST_VBDH_CONFIG.KHTHTT.tenHoSo} ${entityId}`);
        }
    }, [entityId, visible]);
    ...
    const [tenHoSo, setTenHoSo] = useState(`${CONST_VBDH_CONFIG.KHTHTT.tenHoSo} ${entityId}`);
    ...
    const onOk = async () => {
        ...
        const maHoSo = `${identityCode}.${moment().format("YYMMDD")}.${CONST_VBDH_CONFIG.KHTHTT.maHoSo}${entityId}`;
        ...
        const res = await ThemMoiCongViecVanBanDieuHanh_KeHoachTienHanhCuocThanhKiemTra(data);
