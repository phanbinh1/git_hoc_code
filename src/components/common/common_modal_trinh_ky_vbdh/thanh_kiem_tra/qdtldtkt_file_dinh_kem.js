...
import { ThemMoiCongViecVanBanDieuHanh_QuyetDinhThanhLapDoanThanhKiemTra } from "./../../../../actions/app/van_ban_dieu_hanh";
...
const TrinhKy = ({
    ...
    useEffect(() => {
        if (visible && entityId) {
            ...
            setTenHoSo(`${CONST_VBDH_CONFIG.QDTLDTKT.tenHoSo} ${entityId}`);
        }
    }, [entityId, visible]);
    ...
    const [tenHoSo, setTenHoSo] = useState(`${CONST_VBDH_CONFIG.QDTLDTKT.tenHoSo} ${entityId}`);
    ...
    const onOk = async () => {
        ...
        const maHoSo = `${identityCode}.${moment().format("YYMMDD")}.${CONST_VBDH_CONFIG.QDTLDTKT.maHoSo}${entityId}`;
        ...
        const res = await ThemMoiCongViecVanBanDieuHanh_QuyetDinhThanhLapDoanThanhKiemTra(data);
        if (res.result && res.result.error === 0) {
            message.success({ content: "Đã trình ký văn bản" });
            callback && callback({
                status: true,
                quyetDinhThanhLapDoan: res.quyetDinhThanhLapDoan
            })
            _onCancel();
        }
        else {
