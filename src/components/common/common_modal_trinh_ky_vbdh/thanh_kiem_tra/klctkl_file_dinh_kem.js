...
import { ThemMoiCongViecVanBanDieuHanh_KetLuanCuocThanhKiemTra } from "./../../../../actions/app/van_ban_dieu_hanh";
...
const TrinhKy = ({
    ...
    useEffect(() => {
        if (visible && entityId) {
            ...
            setTenHoSo(`${CONST_VBDH_CONFIG.QDKLCTKT.tenHoSo} ${entityId}`);
        }
    }, [entityId, visible]);
    ...
    const [tenHoSo, setTenHoSo] = useState(`${CONST_VBDH_CONFIG.QDKLCTKT.tenHoSo} ${entityId}`);
    ...
    const onOk = async () => {
        ...
        const maHoSo = `${identityCode}.${moment().format("YYMMDD")}.${CONST_VBDH_CONFIG.QDKLCTKT.maHoSo}${entityId}`;
        ...
        const res = await ThemMoiCongViecVanBanDieuHanh_KetLuanCuocThanhKiemTra(data);
        if (res.result && res.result.error === 0) {
            message.success({ content: "Đã trình ký văn bản" });
            callback && callback({
                status: true,
                ketLuanCuocThanhTra: res.ketLuanCuocThanhTra
            })
            _onCancel();
        }
        else {
