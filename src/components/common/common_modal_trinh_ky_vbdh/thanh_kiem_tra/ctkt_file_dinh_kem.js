...
import { ThemMoiCongViecVanBanDieuHanh_CuocThanhKiemTra } from "./../../../../actions/app/van_ban_dieu_hanh";
...
const TrinhKy = ({
    ...
    useEffect(() => {
        if (visible && entityId) {
            ...
            setTenHoSo(`${CONST_VBDH_CONFIG.CTKT.tenHoSo} ${entityId}`);
        }
    }, [entityId, visible]);
    ...
    const [tenHoSo, setTenHoSo] = useState(`${CONST_VBDH_CONFIG.CTKT.tenHoSo} ${entityId}`);
    ...
    const onOk = async () => {
        ...
        const maHoSo = `${identityCode}.${moment().format("YYMMDD")}.${CONST_VBDH_CONFIG.CTKT.maHoSo}${entityId}`;
        ...
        const res = await ThemMoiCongViecVanBanDieuHanh_CuocThanhKiemTra(data);
                ...
                <div className="ant-form-inline">
                    ...
                    <Form.Item labelCol={{ span: 8 }} label="Email người nhận" required>
                        <AutoComplete
                            ...
                            dataSource={accounts.map(item => <AutoComplete.Option key={item.id} text={item.email} {...item}>
                                {item.fullName}
                            </AutoComplete.Option>)}
