...
import { ThemMoiCongViecVanBanDieuHanh_KeHoachTaiChinh } from "./../../../../actions/app/van_ban_dieu_hanh";
...
const TrinhKy = ({
    ...
    useEffect(() => {
        if (visible && entityId) {
            ...
            setTenHoSo(`${CONST_VBDH_CONFIG.KHTC.tenHoSo} ${entityId}`);
        }
    }, [entityId, visible]);
    ...
    const [tenHoSo, setTenHoSo] = useState(`${CONST_VBDH_CONFIG.KHTC.tenHoSo} ${entityId}`);
    ...
    const onOk = async () => {
        ...
        const maHoSo = `${identityCode}.${moment().format("YYMMDD")}.${CONST_VBDH_CONFIG.KHTC.maHoSo}${entityId}`;
        ...
        const res = await ThemMoiCongViecVanBanDieuHanh_KeHoachTaiChinh(data);
