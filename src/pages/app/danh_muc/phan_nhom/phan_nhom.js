...
import PhanNhomComponent from "./../../../../components/app/danh_muc/phan_nhom/phan_nhom";
...
const PhanNhom = ({ ...props }) => {

    const dataSortInit = [
        { key: "id", value: false, sort: 2 },
        { key: "ma", value: true, sort: 1 },
        { key: "tenPhanNhom", value: true, sort: 0 },
    ];
    ...
    const [pageKey] = useState(pageKeys.PAGE_KEY_TCB_PHAN_NHOM);
    ...
    const onSelectRow = (rows_selected = []) => {
        dispatch(act.selectRow(rows_selected, pageKeys.PAGE_KEY_TCB_PHAN_NHOM));
    };
    const getAllRequest = (object = {}) => {
        dispatch(actPhanNhom.getAllRequest({ ...object, pageKey: pageKeys.PAGE_KEY_TCB_PHAN_NHOM }));
    };
    const getOneRequest = (object = {}) => {
        dispatch(actPhanNhom.getOneRequest(object));
    };
    const handleGetOne = (value = {}) => {
        dispatch(actPhanNhom.handleGetOne(value));
    };
    const deleteRequest = (object = {}) => {
        dispatch(actPhanNhom.deleteRequest(object));
    };
    ...
    useEffect(() => {
        if (queryVariable.action) {
            if (queryVariable.action === actID.ACT_ID_TCB_PHAN_NHOM.ACT_CREATE) {
                onCreate();
            }
            if (queryVariable.action === actID.ACT_ID_TCB_PHAN_NHOM.ACT_UPDATE && queryVariable.id) {
                const id = queryVariable.id;
                onEdit(id);
            }
            if (queryVariable.action === actID.ACT_ID_TCB_PHAN_NHOM.ACT_SEARCH) {
                onSearch();
            }
            if (queryVariable.action === actID.ACT_ID_TCB_PHAN_NHOM.ACT_LIST) {
                onList();
            }
        }
    ...
    const handleSearch = () => {
        ...
        objectQueryVariable.action = actID.ACT_ID_TCB_PHAN_NHOM.ACT_SEARCH;
    ...
    const handleCreate = () => {
        let url = `${location.pathname}?`;
        let objectQueryVariable = {}
        objectQueryVariable.action = actID.ACT_ID_TCB_PHAN_NHOM.ACT_CREATE;
        url += main.parseObjectToParams(objectQueryVariable);
        history.push(url);
    };
    ...
    const handleEdit = (id) => {
        let url = `${location.pathname}?`;
        let objectQueryVariable = {}
        objectQueryVariable.action = actID.ACT_ID_TCB_PHAN_NHOM.ACT_UPDATE;
    ...
    const handleList = () => {
        let url = `${location.pathname}?`;
        let objectQueryVariable = {}
        objectQueryVariable.action = actID.ACT_ID_TCB_PHAN_NHOM.ACT_LIST;
        url += main.parseObjectToParams(objectQueryVariable);
        history.push(url);
    };
    ...
    return (
        <React.Fragment>
            {!queryVariable.action && <Redirect to={`${location.pathname}?action=${actID.ACT_ID_TCB_PHAN_NHOM.ACT_LIST}`} />}
            <PhanNhomComponent
                {...props}
                pageKey={pageKey}
