...
import NhomComponent from "./../../../../components/app/danh_muc/nhom/nhom";
...
const Nhom = ({ ...props }) => {
    ...
    const getAllRequest = (object = {}) => {
        dispatch(actNhom.getAllRequest({ ...object, pageKey: pageKeys.PAGE_KEY_TCB_NHOM }));
    };
    const getOneRequest = (object = {}) => {
        dispatch(actNhom.getOneRequest(object));
    };
    const handleGetOne = (value = {}) => {
        dispatch(actNhom.handleGetOne(value));
    };
    const deleteRequest = (object = {}) => {
        dispatch(actNhom.deleteRequest(object));
    };
    ...
    useEffect(() => {
        if (queryVariable.action) {
            if (queryVariable.action === actID.ACT_ID_TCB_NHOM.ACT_CREATE) {
                onCreate();
            }
            if (queryVariable.action === actID.ACT_ID_TCB_NHOM.ACT_UPDATE && queryVariable.id) {
                const id = queryVariable.id;
                onEdit(id);
            }
            if (queryVariable.action === actID.ACT_ID_TCB_NHOM.ACT_SEARCH) {
                onSearch();
            }
            if (queryVariable.action === actID.ACT_ID_TCB_NHOM.ACT_LIST) {
                onList();
            }
        }
    ...
    const handleSearch = () => {
        ...
        objectQueryVariable.action = actID.ACT_ID_TCB_NHOM.ACT_SEARCH;
    ...
    const handleCreate = () => {
        let url = `${location.pathname}?`;
        let objectQueryVariable = {}
        objectQueryVariable.action = actID.ACT_ID_TCB_NHOM.ACT_CREATE;
        url += main.parseObjectToParams(objectQueryVariable);
        history.push(url);
    };
    ...
    const handleEdit = (id) => {
        let url = `${location.pathname}?`;
        let objectQueryVariable = {}
        objectQueryVariable.action = actID.ACT_ID_TCB_NHOM.ACT_UPDATE;
    ...
    const handleList = () => {
        let url = `${location.pathname}?`;
        let objectQueryVariable = {}
        objectQueryVariable.action = actID.ACT_ID_TCB_NHOM.ACT_LIST;
        url += main.parseObjectToParams(objectQueryVariable);
        history.push(url);
    };
    ...
    return (
        <React.Fragment>
            {!queryVariable.action && <Redirect to={`${location.pathname}?action=${actID.ACT_ID_TCB_NHOM.ACT_LIST}`} />}
            <NhomComponent
                {...props}
                pageKey={pageKey}
